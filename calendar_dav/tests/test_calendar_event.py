...
class DavCalendarEventTest(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.mapper = cls.env(user=cls.user)["calendar.event"]
...
@tagged("post_install", "-at_install")
class TestExportCalendarEventMapper(DavCalendarEventTest):
    ...
    def test_export_html_description(self):
        ...
        """
        # Given
        event = self._make_timed_event(description="<p>Hello <b>World</b></p>")
        ...
        self.assertEqual(vevent.x_alt_desc.params.get("FMTTYPE"), ["text/html"])
    ...
    def test_export_attendee_partstat(self):
        ...
        for odoo_state, ical_partstat in state_map.items():
            ...
            matching = [a for a in attendee_props if partner.email in a.value]
            self.assertTrue(
                matching,
    ...
    def test_export_alarms(self):
        ...
        event = self._make_timed_event(
            alarm_ids=[Command.set([email_alarm.id, notif_alarm.id])]
        )
        ...
        # Then — DISPLAY alarm
        self.assertIn(
            "DISPLAY", alarms_by_action, "Expected a VALARM with ACTION:DISPLAY"
        )
...
@tagged("post_install", "-at_install")
class TestImportCalendarEventMapper(DavCalendarEventTest):
    ...
    def test_import_html_description(self):
        ...
        self.assertEqual(
            str(result.description), "<p>This is a <b>bold</b> description.</p>"
        )
...
@tagged("post_install", "-at_install")
class TestRecurrenceCalendarEventMapper(DavCalendarEventTest):
    ...
    def test_export_detached_occurrence_produces_recurrence_id(self):
        ...
        self.assertEqual(
            1,
            len(other_vevents),
            "A VEVENT with RECURRENCE-ID should be exported for the detached occurrence",
        )
        ...
        self.assertFalse(
            hasattr(override_vevent, "rrule"),
            "RRULE should not be present on the detached occurrence VEVENT",
        )
    ...
    def test_export_modified_occurrence_produces_recurrence_id_vevent(self):
        ...
        self.assertEqual(
            len(vevents),
            2,
            "A second VEVENT should be present for the modified occurrence",
        )
        recurrence_id_vevent = next(
            (v for v in vevents if hasattr(v, "recurrence_id")), None
        )
        self.assertIsNotNone(
            recurrence_id_vevent, "A VEVENT with RECURRENCE-ID should be present"
        )
        # Then proper mappind is done for exception.
        paris_tz = pytz.timezone("Europe/Brussels")
        self.assertEqual(
            set(recurrence_id_vevent.contents.keys()),
            {
                "summary",
                "dtend",
                "uid",
        ...
        self.assertEqual(
            recurrence_id_vevent.recurrence_id.value,
            pytz.utc.localize(datetime(2024, 6, 16, 10, 0)).astimezone(paris_tz),
        )
        self.assertEqual(recurrence_id_vevent.summary.value, "Modified Occurrence")
        self.assertEqual(
            recurrence_id_vevent.dtstart.value,
            pytz.utc.localize(datetime(2024, 6, 16, 10, 0)).astimezone(paris_tz),
        )
        self.assertEqual(
            recurrence_id_vevent.dtend.value,
            pytz.utc.localize(datetime(2024, 6, 16, 11, 0)).astimezone(paris_tz),
        )
        self.assertEqual(
            recurrence_id_vevent.organizer.value, "mailto:demo@example.com"
        )
    ...
    def test_import_multiple_exdates_removes_occurrences(self):
        ...
        self.assertFalse(
            excluded_dates & active_dates, "No occurrence should exist on EXDATEd dates"
        )
    ...
    def test_import_exdate_restores_previously_excluded_occurrence(self):
        ...
        self.assertIn(
            restored_date,
            {e.start.date() for e in all_events},
    ...
    def test_import_exdate_restores_previously_hard_deleted_occurrence(self):
        ...
        self.assertIn(
            restored_date,
            {e.start for e in active_events},
    ...
    def test_import_recurrence_id_vevent_updates_occurrence(self):
        ...
        override_vevent.add("dtstart").value = datetime(
            2024, 6, 16, 14, 0, 0, tzinfo=pytz.utc
        )
        override_vevent.add("dtend").value = datetime(
            2024, 6, 16, 15, 0, 0, tzinfo=pytz.utc
        )
        override_vevent.add("recurrence-id").value = datetime(
            2024, 6, 16, 10, 0, 0, tzinfo=pytz.utc
        )
        ...
        self.assertEqual(
            datetime(2024, 6, 16, 14, 0, 0, tzinfo=pytz.utc),
            second.start.replace(tzinfo=pytz.utc),
        )
        self.assertEqual(
            datetime(2024, 6, 16, 15, 0, 0, tzinfo=pytz.utc),
            second.stop.replace(tzinfo=pytz.utc),
        )
    ...
    def test_import_update_non_recurring_to_daily_recurring(self):
        ...
        for i, occ in enumerate(occurrences):
            ...
            self.assertEqual(
                expected_start.replace(tzinfo=None), occ.start.replace(tzinfo=None)
            )
    ...
    def test_import_update_daily_recurring_to_non_recurring(self):
        ...
        self.assertFalse(
            result.recurrence_id or result.recurrence_id.calendar_event_ids
        )
        ...
        self.assertEqual(
            existing_event.start.replace(tzinfo=None), result.start.replace(tzinfo=None)
        )
        self.assertEqual(
            existing_event.stop.replace(tzinfo=None), result.stop.replace(tzinfo=None)
        )
