...
class Meeting(models.Model):
    ...
    is_base_event = fields.Boolean(
        string="Is Base Event", compute="_compute_is_base_event", store=True
    )

    @api.depends(
        "recurrency",
        "recurrence_id",
        "recurrence_id.base_event_id",
    ...
    def _compute_is_base_event(self):
        ...
        """
        for record in self:
            record.is_base_event = (
                not record.recurrence_id or record.recurrence_id.base_event_id == record
            )
    ...
    def _analyze_recurrence(self, record):
        ...
        outlier_events = all_events.filtered(
            lambda e: e.start not in theoretical_starts
        )
    ...
    def _compute_modified_events(self, base_event):
        ...
        def _modified(ev):
            return ev.id != base_event.id and any(
                getattr(ev, field) != getattr(base_event, field) for field in fields
            )
    ...
    def _parse_attendees(self, record, attendee_list) -> tuple[list, list]:
        ...
        existing_attendees = (
            {att.partner_id.id: att for att in record.attendee_ids} if record else {}
        )
        ...
        for attendee_prop in attendee_list:
            ...
            if not partner:
                ...
                partner = self.env["res.partner"].create({"name": cn, "email": email})

            partner_commands.append(Command.link(partner.id))

            existing_attendee = existing_attendees.get(partner.id)
            if existing_attendee:
                attendee_commands.append(
                    Command.update(existing_attendee.id, {"state": odoo_state})
                )
            else:
                attendee_commands.append(
                    Command.create({"partner_id": partner.id, "state": odoo_state})
                )
    ...
    def _get_or_create_category(self, name: str):
        ...
        """
        tag = self.env["calendar.event.type"].search([("name", "=", name)], limit=1)
        if not tag:
            try:
    ...
    def _apply_categories(self, vevent):
        ...
        categ_ids = [
            self._get_or_create_category(n.strip()).id for n in tag_names if n.strip()
        ]
        if categ_ids:
    ...
    def _apply_attendees(self, vevent, record):
        ...
        if hasattr(vevent, "attendee_list"):
            partner_commands, attendee_commands = self._parse_attendees(
                record, vevent.attendee_list
            )
            if partner_commands:
    ...
    def _get_or_create_alarm(self, alarm_type, duration, interval, total_minutes, name):
    ...
    def _apply_rrule(self, vevent, record):
        ...
        if hasattr(vevent, "rrule"):
            ...
            elif new_rrule != record.rrule and _rrule_parse(
                new_rrule, None
            ) != _rrule_parse(record.rrule, None):
    ...
    def _apply_exdates(self, vevent, record, collection):
        ...
        dates_to_exclude = [
            self._datetime_to_db(dt) for dt in (new_exdates - current_exdates)
        ]
        if dates_to_exclude:
        ...
        dates_to_restore = [
            self._datetime_to_db(dt) for dt in (current_exdates - new_exdates)
        ]
        if dates_to_restore:
            ...
            missing_dates = set(dates_to_restore) - {
                self._datetime_to_db(e.start) for e in matching
            }
            for missing_start in missing_dates:
                ...
                values.update(
                    self.extract_field_mapping_from_vobject(vevent, collection)
                )
    ...
    def _export_dates(self, vevent, record):
        ...
        if record.allday:
            ...
            vevent.add("dtstart").value = self._datetime_from_db(
                record.start, record.event_tz
            )
            vevent.add("dtend").value = self._datetime_from_db(
                record.stop, record.event_tz
            )
    ...
    def _export_organizer(self, vevent, record):
        ...
        org_email = organizer.email or f"noreply+{organizer.id}@odoo.local"
    ...
    def _export_recurrence(self, vevent, record, collection, cal, original_start):
        ...
        if record.is_base_event:
            ...
            for outlier_event, original_start in outliers.items():
                self.to_vobject(
                    outlier_event, collection, cal=cal, original_start=original_start
                )
                    ...
                    .search(
                        [
                            ...
                            (
                                "start",
                                "=",
                                self._datetime_to_db(vevent.recurrence_id.value),
                            ),
                        ]
                    )
                )
